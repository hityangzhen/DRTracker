#ifndef __CORE_WRAPPER_HPP
#define __CORE_WRAPPER_HPP
...
//Function prototype arguments , including the type and size of each 
//argument specified.
#define PARGS_0
...
//HandlerType is a function pointer
...
#define WRAPPER_TEMPLATE(NUM_ARGS)										\
	...
	public:																\
		...
		void set_ori_funptr(AFUNPTR ori_funptr) {						\
			ori_funptr_=ori_funptr;										\
		}																\
...
#define WRAPPER(name_,func_,lib_,group_,sig_)							\
	class WRAPPER_CLASS(name_): 										\
						public Wrapper<WRAPPER_CLASS(name_),sig_> {		\
	public:																\
		~WRAPPER_CLASS(name_)() {}										\
		WRAPPER_CLASS(name_)() {}										\
																		\
	...
	do {																\
		...
		if(IMG_Name(img).find(wrapper->lib()) != std::string::npos) {	\
			DEBUG_FMT_PRINT_SAFE("Activate wrapper %s in %s\n",			\
				wrapper->func().c_str(),wrapper->lib().c_str());		\
...
WRAPPER(PthreadRwlockTryRdlock,"pthread_rwlock_tryrdlock","libpthread.so","pthread",int(pthread_rwlock_t *));
WRAPPER(PthreadRwlockRdlock,"pthread_rwlock_rdlock","libpthread.so","pthread",int(pthread_rwlock_t *));
WRAPPER(PthreadRwlockTryWrlock,"pthread_rwlock_trywrlock","libpthread.so","pthread",int(pthread_rwlock_t *));
WRAPPER(PthreadRwlockWrlock,"pthread_rwlock_wrlock","libpthread.so","pthread",int(pthread_rwlock_t *));
WRAPPER(PthreadRwlockUnlock,"pthread_rwlock_unlock","libpthread.so","pthread",int(pthread_rwlock_t *));
...
WRAPPER(SemInit,"sem_init","libpthread.so","pthread",int(sem_t *,int ,unsigned int));
WRAPPER(SemPost,"sem_post","libpthread.so","pthread",int(sem_t *));
WRAPPER(SemWait,"sem_wait","libpthread.so","pthread",int(sem_t *));


#endif /* __CORE_WRAPPER_HPP */
